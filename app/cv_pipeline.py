...
class LocalFileStorage:
    def save(self, tenant_id: str, application_id: str, version: int, filename: str, content: bytes) -> tuple[str, bytes | None]:
...
def validate_cv(filename: str, mime_type: str, size_bytes: int) -> str:
    ...
    if ext not in ALLOWED_EXT:
        raise ValueError("Formato inválido. Sólo PDF/JPG/PNG")
    if mime_type not in ALLOWED_MIME and ext not in ALLOWED_EXT:
        raise ValueError("MIME inválido")
    if size_bytes <= 0 or size_bytes > 20 * 1024 * 1024:
        raise ValueError("El CV excede 20MB")
    return ext
...
def next_cv_version(db, application_id):
    current = db.execute(
